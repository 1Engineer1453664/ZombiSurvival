...
GM.WeaponQualities = {
    {"Прочное", 1.09, "Настроенное"},
    {"Закаленное", 1.19, "Модифицированное"},
    {"Совершенное", 1.35, "Реформированное"}
}
...
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_FIRE_DELAY,                 "Задержка Выстрела",       false,  {Delay = true})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_RELOAD_SPEED,               "Скорость Перезарядки",     false,  {ReloadSpeed = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_CLIP_SIZE,                  "Размер Магазина",          true,   {ClipSize = true}).ReqClip = true
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_MELEE_RANGE,                "Дальность Ближнего Боя",   false,  {MeleeRange = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_MELEE_SIZE,                 "Радиус Ближнего Боя",      false,  {MeleeSize = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_MELEE_IMPACT_DELAY,         "Задержка Удара",           false,  {SwingTime = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_PROJECTILE_VELOCITY,        "Скорость Полета Снаряда",  false,  {ProjVelocity = true})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_SHORT_TEAM_HEAT,            "Нагрев Оружия",            false,  {HeatBuildShort = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_SHOT_COUNT,                 "Кол-во Дробинок",          true,   {NumShots = true})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_BULLET_PIERCES,             "Пробитие Пули",            true,   {Pierces = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_MAXIMUM_MINES,              "Макс. Мин на поле",        true,   {MaxMines = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_MAX_DISTANCE,               "Макс. Дистанция",          false,  {MaxDistance = false})
...
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_HEALRANGE,                  "Дальность Лечения",        false,  {HealRange = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_HEALCOOLDOWN,               "Перезарядка Лечения",      false,  {Delay = true})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_BUFF_DURATION,              "Длительность Баффа",       false,  {BuffDuration = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_LEG_DAMAGE,                 "Сила Замедления",          false,  {LegDamage = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_REPAIR,                     "Эффективность Ремонта",    false,  {Repair = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_TURRET_SPREAD,              "Разброс Турели",           false,  {TurretSpread = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_HEALING,                    "Количество Лечения",       false,  {Heal = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_HEADSHOT_MULTI,             "Бонус Урона в Голову",     false,  {HeadshotMulti = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_MELEE_KNOCK,                "Отбрасывание",             false,  {MeleeKnockBack = false})
...
function GM:CreateWeaponOfQuality(i, orig, quality, classname, branch)
	...
	if wept.PrimaryRemantleModifier then
		...
		table.insert(remantledescs, "+" .. ((quality[2]-1) * 100) .. "% " .. "Урон")
	end
